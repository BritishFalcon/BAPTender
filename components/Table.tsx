...
export default function UserBACStatusTable() {
  ...
  const tableData = Object.keys(state.states).map((uid) => {
    ...
    if (!points || points.length === 0) {
      return { uid, displayName, currentBac: "0.000", sobrietyTime: "Sober", isSelf: uid === state.self.id };
    }

    const lastPoint = points[points.length - 1];
    const hoursElapsed = (now.getTime() - new Date(lastPoint.time).getTime()) / (1000 * 60 * 60);
    ...
    const sobrietyDate = new Date(now.getTime() + sobrietyTimeMinutes * 60000);

    // 3. Format the date into a nice, readable time string (e.g., "6:31 AM").
    // We only do this if they are not already sober.
    const sobrietyTimeString = currentBac > 0
      ? sobrietyDate.toLocaleTimeString(navigator.language, { hour: 'numeric', minute: '2-digit' })
      : "Sober";
