...
if (document.cookie.includes("username=")) {
    ...
    document.getElementById("userForm").addEventListener("submit", function(e) {
        ...
        my_weight = document.getElementById("weight").value;
        document.cookie = "sex=" + document.getElementById("sex").value;
        my_sex = document.getElementById("sex").value;
        loaded = true;
        document.getElementById("userFormDiv").style.display = "none";  // Hide the form
        document.getElementById("main").style.display = "block";  // Show the main page

        // Get the users' drinks from the server
        drinks = socket.emit('new_user', [my_username, my_weight, my_sex]);
        socket.emit('get_data')
        autoUpdate()
    });
}

function calculateBACOverTime(username, drinks, bodyWeightInKg, sex) {
    const metabolismRate = 0.015;  // 0.015 g/100mL/hour // FIX THIS
    const widmarkFactor = (sex=='male') ? 0.68 : 0.55;
...
function autoUpdate() {
    ...
    for (const username in drinks) {
        ...
        const sex = users[username][1];

        const { timeLabels, bacValues } = calculateBACOverTime(username, user_drinks, weight, sex);
